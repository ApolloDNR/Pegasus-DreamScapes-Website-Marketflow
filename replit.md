...
## Navigation grouping (locked v1.0.1 + Task #148)

- **Desktop header (Task #148)**: Four primary items — **Development** (dropdown parent → Projects, Development Path), **Strategy Lab**, **MarketFlow**, **About** — plus the **More** dropdown sourced from `NAV_MORE`. Projects moved out of NAV_PRIMARY into the Development dropdown; it remains in the footer Services column as an extra link. Brand wordmark left (illustrated Pegasus mark + Cinzel "PEGASUS DREAMSCAPES" + Montserrat "THE DEAL ARCHITECT" subtitle), "Submit a Property" copper CTA right. Header is **transparent over the hero** and only fades to a frosted-glass plate after ~80px of scroll. **Glass budget (guardrail #4): 3 surfaces max** — (1) scrolled nav plate, (2) hero floating Sample card, (3) MarketFlow gated landing intro. Everything else uses solid panels.
