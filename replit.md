...
- **Public routes locked**: `/`, `/sell`, `/invest`, `/projects`, `/projects/:slug`, `/contact`, `/marketflow`, `/deal-blueprint`, `/snapshot/:token`, plus the existing `/resources` library. Until the dedicated Education and Vendor Network surfaces ship, the homepage role-router and Peggy quick-prompts route "Learn strategies" → `/resources` and "Vendor or operator" → `/contact` so no link is a dead end.
