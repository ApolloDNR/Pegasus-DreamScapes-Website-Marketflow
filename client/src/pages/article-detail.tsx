  ...
  const renderContent = (content: string) => {
    ...
    const flushList = () => {
      if (listItems.length > 0) {
        ...
        if (listType === 'ol') {
          elements.push(
            <ol key={`list-${elements.length}`} className="list-decimal pl-6 mb-6 space-y-2 text-muted-foreground leading-relaxed">
              {items}
            </ol>,
          );
        } else {
          elements.push(
            <ul key={`list-${elements.length}`} className="list-disc pl-6 mb-6 space-y-2 text-muted-foreground leading-relaxed">
              {items}
            </ul>,
          );
        }
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed.startsWith('# ')) {
        flushList();
        elements.push(
          <h1 key={index} className="font-serif text-3xl sm:text-4xl font-semibold tracking-[-0.02em] mb-6 mt-10 first:mt-0">
            {trimmed.slice(2)}
          </h1>,
        );
        ...
        flushList();
        elements.push(
          <h2 key={index} className="font-serif text-2xl sm:text-3xl font-semibold tracking-[-0.01em] mb-4 mt-10">
            {trimmed.slice(3)}
          </h2>,
        );
      } else if (trimmed.startsWith('### ')) {
        flushList();
        elements.push(
          <h3 key={index} className="font-serif text-xl font-semibold mb-3 mt-7">
            {trimmed.slice(4)}
          </h3>,
        );
  ...
  return (
    <div className="min-h-screen pt-20">
      <article className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="max-w-none" data-testid="article-content">
