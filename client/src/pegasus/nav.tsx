...
export function NavBar({ go, route, theme, toggleTheme, scrolled, openPeggy }:
  ...
  const navigate = (item: PremiumItem) => {
    setMenuOpen(false);
    setMoreOpen(false);
  ...
  const isActive = (item: PremiumItem) => location === itemUrl(item);
    ...
    <>
      <nav ref={navRef} className="fixed top-0 inset-x-0 z-40">
        ...
        <div className={`relative max-w-[1440px] mx-auto px-6 lg:px-10 flex items-center justify-between transition-all duration-500 ${scrolled ? 'h-[74px]' : 'h-24'} ${text}`}>
          ...
          <div className="hidden min-[1340px]:flex items-center gap-3 min-[1500px]:gap-5 pg-label !text-[9px] !tracking-[0.16em]">
            {PREMIUM_NAVIGATION.primary.map((item) => (
              <button key={item.label} type="button" onClick={() => navigate(item)} aria-current={isActive(item) ? 'page' : undefined}
            ...
            <div className="relative">
              <button ref={moreButtonRef} type="button" aria-expanded={moreOpen} aria-haspopup="menu" onClick={() => setMoreOpen((open) => !open)}
              ...
              {moreOpen && (
                <div ref={moreRef} role="menu" aria-label="More Pegasus pages" className="px-more-menu">
                    ...
                    {PREMIUM_NAVIGATION.more.map((group) => (
                      <section key={group.label}>
                        ...
                        {group.items.map((item) => (
                          <button key={item.label} type="button" role="menuitem" onClick={() => navigate(item)} aria-current={isActive(item) ? 'page' : undefined}>
          ...
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <button type="button" onClick={() => setLocation('/bring-an-opportunity')} className={`pg-nav-cta hidden sm:inline-flex ${overHero ? 'pg-nav-cta-hero' : 'pg-nav-cta-scrolled'} px-5 lg:px-6 py-3 pg-label !text-[10px] !tracking-[0.2em]`}>Bring an Opportunity</button>
            <button ref={menuButtonRef} type="button" aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} aria-controls="mobile-menu" onClick={toggleMenu} style={{ touchAction: 'manipulation' }} className="min-[1340px]:hidden relative z-10 -mr-2 p-2.5">
      ...
      <div ref={menuRef} id="mobile-menu" role="dialog" aria-modal={menuOpen ? 'true' : undefined} aria-label="Primary navigation" aria-hidden={!menuOpen} tabIndex={-1} {...(!menuOpen ? { inert: '' } : {})}
        style={{ transform: 'translateZ(0)', WebkitTransform: 'translateZ(0)' }} className={`min-[1340px]:hidden fixed inset-0 z-[100] bg-[var(--bg-2)] transition-opacity duration-300 ease-out ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="h-full px-6 pt-24 pb-10 flex flex-col text-[var(--text)] overflow-y-auto overscroll-contain">
          <div className="flex flex-col gap-3">
            <button type="button" onClick={() => navigate({ label: 'Bring an Opportunity', url: '/bring-an-opportunity' })} data-menu-initial-focus className="btn-primary px-6 py-4 pg-label !text-[10px] !tracking-[0.2em] text-center inline-flex items-center justify-center gap-2.5 group">Bring an Opportunity <ArrowRight className="w-3.5 h-3.5" /></button>
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => navigate({ label: 'Strategy Lab', route: 'strategylab' })} className="btn-line px-4 py-4 pg-label !text-[9px] text-center">Strategy Lab</button>
              <button type="button" onClick={() => navigate({ label: 'MarketFlow', route: 'marketflow' })} className="btn-line px-4 py-4 pg-label !text-[9px] text-center">MarketFlow</button>
            </div>
          </div>
          ...
          <div className="mt-7 flex flex-col">
            {PREMIUM_NAVIGATION.mobile.map((item) => (
              <div key={item.label} className="nav-m-acc"><button type="button" className="nav-m-acc-trigger w-full" onClick={() => navigate(item)}>{item.label}{item.badge && <span className="px-nav-badge">{item.badge}</span>}</button></div>
            ))}
          </div>
        </div>
      </div>
