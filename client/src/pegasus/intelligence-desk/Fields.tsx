...
export function InputField({ label, value, onChange, unit, origin, error, numeric = false }: { label: string; value: string; onChange: (value: string) => void; unit?: string; origin?: string; error?: string; numeric?: boolean }) {
  const id = React.useId();
  return <div className="id-field"><label htmlFor={id}>{label}</label><div className="id-input-wrap"><input id={id} aria-label={label} aria-invalid={Boolean(error)} aria-describedby={`${id}-note`} inputMode={numeric ? 'decimal' : undefined} value={value} onChange={event => onChange(event.target.value)} autoComplete="off" maxLength={180} />{unit && <span aria-hidden="true">{unit}</span>}</div><small id={`${id}-note`} className={error ? 'id-error' : ''}>{error || `${origin || 'Visitor entered'}${unit ? ` · ${unit}` : ''}`}</small></div>;
}
...
export function DraftInput({ field, draft, onChange, error }: { field: NumericField; draft: Draft; onChange: (key: DraftField, value: string) => void; error?: string }) {
  return <InputField label={FIELD_LABELS[field]} value={draft[field]} unit={FIELD_UNITS[field]} error={error} origin={fieldOrigin(draft, field)} numeric onChange={value => onChange(field, value)} />;
}
...
export function DraftSelect({ field, label, draft, onChange }: { field: keyof typeof OPTIONS; label: string; draft: Draft; onChange: (key: DraftField, value: string) => void }) {
  const id = React.useId();
  return <div className="id-field"><label htmlFor={id}>{label}</label><select id={id} aria-label={label} value={draft[field]} onChange={event => onChange(field, event.target.value)}>{OPTIONS[field].map(value => <option key={value}>{value}</option>)}</select><small>{fieldOrigin(draft, field)}</small></div>;
}
