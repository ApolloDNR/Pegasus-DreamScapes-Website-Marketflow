...
import { scenarioDraft } from './scenario-model';
  ...
  return <>
    ...
    <div className="id-scenario-columns">{runs.map(run => {
      ...
      return <section key={run.id} className={active === run.id ? 'is-active' : ''} aria-label={`${SCENARIO_NAMES[run.id]} comparison`}>
        ...
        <p className="id-scenario-description">{!variant ? 'The starting point for every comparison. Edit Base to refine the shared property model.' : differences.length ? 'Results below use this scenario’s current overrides. Compare the exact changes below.' : 'This scenario currently matches Base. Edit its assumptions or choose a preset to explore a different case.'}</p>
        {run.analysis.status === 'ready' ? <><dl><div><dt>Cash required</dt><dd>{money(run.analysis.presentation.totalCashIn)}</dd></div><div><dt>Highest modeled fit</dt><dd>{laneName(run.analysis.presentation.lanes[0])}</dd></div><div><dt>Monthly rental cash flow</dt><dd>{money(run.analysis.rentalMetrics.monthlyCashFlow)}</dd></div></dl>{run.analysis.rentalUnavailableReason && <p className="id-caption">{run.analysis.rentalUnavailableReason}</p>}</> : <p>Needs valid scenario inputs.</p>}
    ...
    <p className="id-caption">Presets apply only when you choose them. Missing and out-of-range inputs stay unchanged. Applying a preset replaces that scenario’s overrides; Undo restores the prior state.</p>
